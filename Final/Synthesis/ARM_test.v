...
module ARM(
    ...
    output [31:0] debug_r0,
    output [31:0] debug_r1,
    output [31:0] debug_r2,
    output [31:0] debug_r3,
    output [31:0] debug_r4,
    output [31:0] debug_r5,
    output [31:0] debug_r6,
    output [31:0] debug_r7,
    ...
    wire [31:0] rf_debug_r0;
    wire [31:0] rf_debug_r1;
    wire [31:0] rf_debug_r2;
    wire [31:0] rf_debug_r3;

    wire carry_forwarded;

    wire [31:0] rf_debug_r4;
    wire [31:0] rf_debug_r5;
    wire [31:0] rf_debug_r6;
    wire [31:0] rf_debug_r7;
    ...
    reg EX_MEM_N_flag, EX_MEM_Z_flag, EX_MEM_C_flag, EX_MEM_V_flag;
    reg MEM_WB_N_flag, MEM_WB_Z_flag, MEM_WB_C_flag, MEM_WB_V_flag;
   ...
   assign debug_instr = instruction;
    ...
    RegisterFile rf(
        ...
        .N_flag(MEM_WB_N_flag),
        .Z_flag(MEM_WB_Z_flag),
        .C_flag(MEM_WB_C_flag),
        .V_flag(MEM_WB_V_flag),
        ...
        .debug_r0(rf_debug_r0),
        .debug_r1(rf_debug_r1),
        .debug_r2(rf_debug_r2),
        .debug_r3(rf_debug_r3),

        .debug_r4(rf_debug_r4),
        .debug_r5(rf_debug_r5),
        .debug_r6(rf_debug_r6),
        .debug_r7(rf_debug_r7)

    );


    assign debug_r0 = rf_debug_r0;
	assign debug_r1 = rf_debug_r1;
	assign debug_r2 = rf_debug_r2;
	assign debug_r3 = rf_debug_r3;

	assign debug_r4 = rf_debug_r4;
	assign debug_r5 = rf_debug_r5;
	assign debug_r6 = rf_debug_r6;
	assign debug_r7 = rf_debug_r7;

   
    wire [31:0] alu_out;
    wire alu_cout;

    assign carry_forwarded =
        EX_MEM_CPSR_WriteEn
        ? alu_C_flag
        : CPSR_out[29];
    ...
    assign N_flag = CPSR_out[31];
    assign Z_flag = CPSR_out[30];
    assign C_flag = CPSR_out[29];
    assign V_flag = CPSR_out[28];
    ...
    .IF_ID_Rn(Rn),
    .IF_ID_Rm(Rm),
    .IF_ID_opcode(IF_ID_instr[15:11]),
    .ID_EX_Rd(ID_EX_Rd),
    .EX_MEM_Rd(EX_MEM_Rd),
    .MEM_WB_Rd(MEM_WB_Rd),
    ...
    .ID_EX_CPSR_WriteEn(ID_EX_CPSR_WriteEn),
    .EX_MEM_CPSR_WriteEn(EX_MEM_CPSR_WriteEn),
    .MEM_WB_CPSR_WriteEn(MEM_WB_CPSR_WriteEn),
    ...
    begin
        ...
        begin
            ...
            EX_MEM_addr <= ID_EX_A;
            EX_MEM_N_flag <= alu_N_flag;
            EX_MEM_Z_flag <= alu_Z_flag;
            EX_MEM_C_flag <= alu_C_flag;
            EX_MEM_V_flag <= alu_V_flag;
    ...
    begin
        ...
        begin
            ...
            MEM_WB_N_flag <= EX_MEM_N_flag;
            MEM_WB_Z_flag <= EX_MEM_Z_flag;
            MEM_WB_C_flag <= EX_MEM_C_flag;
            MEM_WB_V_flag <= EX_MEM_V_flag;
...
module HazardDetectionUnit(
    ...
    input ID_EX_CPSR_WriteEn,
    input EX_MEM_CPSR_WriteEn,
    input MEM_WB_CPSR_WriteEn,
    ...
    wire carry_dep;
    assign carry_dep =
           (IF_ID_opcode == 5'b00011) || // ADC
           (IF_ID_opcode == 5'b01101);   // SBC
...
module alu #(
	...
	begin
		...
		case (opcode)
			...
			5'b00111: begin result = b;  cout = 0; end                // MOV
			5'b01000: begin result = ~b; cout = 0; end                // MVN
			5'b01001: begin result = sub_out; cout = sub_cout; end    // CMP
			5'b01010: begin result = add_out; cout = add_cout; end    // CMN
			5'b01011: begin result = bic_out; cout = 0; end           // BIC
			5'b01100: begin result = rsb_out; cout = rsb_cout; end    // RSBS
...
module RegisterFile (
    ...
    assign debug_r0 = RF[0];
    assign debug_r1 = RF[1];
    assign debug_r2 = RF[2];
    assign debug_r3 = RF[3];

    assign debug_r4 = RF[4];
    assign debug_r5 = RF[5];
    assign debug_r6 = RF[6];
    assign debug_r7 = RF[7];
...
assign next_pc = (branch_en == 1'b1) ? branch_addr : (pc_addr + 8'd1);
...
module InstructionMemory(
   ...
   // end
        ...
        // MOVI r0, #7
        // MOVI r1, #6
        // ADDS r2, r0, r1       r2 = 7 + 6 = 13
        // SUBS r3, r2, r1       r3 = 13 - 6 = 7
        // NOP
        // =====================================================================

        // mem[0] = 16'b11001_1_000_000_0111;   // MOVI r0, #7
        // mem[1] = 16'b11001_1_001_000_0110;   // MOVI r1, #6
        ...
        // mem[7] = 16'b11111_00000000000;      // ENDD (HALT)
        ...
        // mem[5]  = 16'b01100_0_101_001_000_0;  // RSBS r5,r1,r0
	    ...
	    // mem[15] = 16'b11111_00000000000;     //END
        ...
        // mem[44] = 16'b11001_1_010_000_1001;   // MOVI r2,#-7 (EXECUTES)
        // mem[45] = 16'b00000_0_000_000_000_0;  // NOP
        // mem[46] = 16'b00000_0_000_000_000_0;  // NOP
        // mem[47] = 16'b11111_00000000000;      // ENDD

    // end

    always @(*) begin
        case (addr)
            8'd0: instruction = 16'b11001_1_000_000_0101; // MOVI r0, #5
            8'd1: instruction = 16'b11001_1_001_000_0011; // MOVI r1, #3
            8'd2: instruction = 16'b00001_0_010_000_001_0; // ADDS r2, r0, r1
            8'd3: instruction = 16'b00010_0_011_010_001_0; // SUBS r3, r2, r1
            8'd4: instruction = 16'b11111_00000000000;     // ENDD
            default: instruction = 16'b0; // NOP
        endcase
    end
